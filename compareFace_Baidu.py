...
for face in faceDict:
	print 'now calc No.%d, %s' %(count,face)
    ...
    for face_compare in faceDict:
    	if face_compare != face:
			...
			request_compare = urllib2.Request(url=request_url, data=params_compare)
			request_compare.add_header('Content-Type', 'application/x-www-form-urlencoded')
			response_compare = urllib2.urlopen(request_compare)
			content_compare = response_compare.read()

			if content_compare:
				times_FMR = times_FMR + 1
				result_score_compare = content_compare[48:63]
				if result_score_compare >= 80:
...
for face in faceDict:
	...
	for face_compare in faceDict:
		if face_compare != face:
			...
			params_compare = {"images":image1_encoding + "," + image_compare_encoding}
			# params_compare = urllib2.Request(url=request_url,data=params_compare)
			params_compare = urllib.urlencode(params_compare)
			...
			if content:
				...
				if result_score >= 80:
					count_FMR = count_FMR + 1
FMR = count_FMR / times_FMR

# print "after %f times match test,FNMR is: "%times_FNMR,FNMR
print "after %f times match test,FMR  is: "%times_FMR,FMR
# print "FMR  is: ",FMR